...
- **Improve docs** — README, `docs/`, and the per-package READMEs are all fair game.
...
The README's architecture section and the files under `docs/` are the best places to orient yourself before touching `packages/core/src/`.
...
- **No new code paths producing `ILinkReferenceDefinitionState`.** Reference link/image definitions round-trip through paragraph state — see `markdownToState.ts`.
...
- **Architecture deep-dives** → Start with the README's architecture section and the files under `docs/`.
